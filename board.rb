...
module Chess
  class Board
    ...
    # pieces is a hash of {[row:(0..7), col:(0..7)] => Chess::Piece}
    def initialize(width=8, height=8, pieces=nil)
      pieces = pieces || Chess::Board.layout_to_pieces(CLASSIC)
      @width, @height, @pieces = width, height, pieces
      pieces.each { |pos, piece| piece.square = self[*pos] }
    end
  
    def [](row_or_str, col=nil)
      return Chess::Square.parse(self, row_or_str.to_s) unless col
      Chess::Square.new(self, [row_or_str, col])
    end
  ...
  class Square
    ...
    def piece=(new_piece)
      board.pieces[[row, col]] = new_piece
    end
...
if __FILE__ == $0
  board = Chess::Board.classic  
  puts board.to_s(axes: true)
  puts "a8:"
  board[:a8].piece.moves.each { |m| puts m }
  puts "a7:"
  board[:a7].piece.moves.each { |m| puts m }
  board[:a7].die!
  puts "a8 after a7 death:"
  board[:a8].piece.moves.each { |m| puts m.to_s(fmt_piece: :to_s) }
